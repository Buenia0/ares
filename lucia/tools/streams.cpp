auto StreamManager::construct() -> void {
  ...
  streamList.onToggle([&](auto cell) {
    if(auto item = streamList.selected()) {
      if(auto stream = item.attribute<velvet::Node::Stream>("node")) {
...
auto StreamManager::reload() -> void {
  ...
  for(auto stream : velvet::Node::enumerate<velvet::Node::Stream>(emulator->root)) {
    TableViewItem item{&streamList};
    item.setAttribute<velvet::Node::Stream>("node", stream);
